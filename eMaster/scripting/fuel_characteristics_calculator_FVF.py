...
def mass_solver(p_fuel, fvf, num_assemblies):
    # Find number of fuel pins
    num_pins = num_assemblies*271
    # Find total volume of fuel
    fuel_height = 81.3
    ph = 16.1420
    pin_d = math.sqrt(fvf*math.sqrt(3)*0.5*pow(ph,2)/(math.pi*0.25*217))
    v_pin = fuel_height*(math.pi/4)*pin_d**2
    v_fuel = num_pins*v_pin
...
def solve_core_compositionANDcosts():
    ...
    sd_inner = float(input('Enter FVF for inner core: '))
    sd_outer = float(input('Enter FVF for outer core: '))
